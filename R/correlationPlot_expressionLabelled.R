# This is an adaptation of the function built into the package BayesianTools
# This is itself GPL 3.0 licensed.
# Source: https://github.com/florianhartig/BayesianTools/blob/661e126ace2e4194128012d578c51a9ffd6137e4/BayesianTools/R/plotCorrelationDensity.r#L79
...
correlationPlot_expressionLabelled<- function(mat, density = "smooth", thin = "auto", method = "pearson", whichParameters = NULL, scaleCorText = T, ...){
  ...
  if (density == "smooth"){
    return(pairs(mat, labels = parse(text = colnames(mat)), lower.panel=function(...) {par(new=TRUE);IDPmisc::ipanel.smooth(...)}, diag.panel=panel.hist.dens, upper.panel=panel.cor))
  }else if (density == "corellipseCor"){
    return(pairs(mat, labels = parse(text = colnames(mat)), lower.panel=plotEllipse, diag.panel=panel.hist.dens, upper.panel=panel.cor))
  }else if (density == "ellipse"){
    correlationEllipse(mat)
  }else if (density == F){
    return(pairs(mat, , labels = parse(text = colnames(mat)), lower.panel=panel.cor, diag.panel=panel.hist.dens, upper.panel=panel.cor))
