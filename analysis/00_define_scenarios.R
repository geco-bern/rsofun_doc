...
setup_rsofun_calibration <- function(scenario){
  ...
  if (scenario %in% c(120,121,122,123,124,125,126,127,1270,128,129,  # the 120s are reruns from 2025-09-17 that lower the prior bound of beta_unitcostratio to 0.01))
    ...
    if (scenario %in% c(120,121,220,221,320,321,420,421)){
    ...
    if (scenario %in% c(124,224,324,424)){
    ...
    if (scenario %in% c(125,225,325,425)){
    ...
    if (scenario %in% c(420,421,422,423,424,425,426,427,428)){         # the 420s are reruns from 2025-09-20 that add a multiplicative bias to gpp with a normal prior around 1))
      par_to_estimate$err_bigD13C     <- list(lower = 0.01, upper = 15, init = 0.8)
      par_to_estimate$errbias_bigD13C <- NULL
      par_to_estimate$errbias_vj      <- NULL
      par_to_estimate$errscale_gpp    <- list(mean = 1, sd = 0.25, lower = 0.25, upper = 3) # truncated normal
    }
  ...
  if (scenario %in% c(115, 125,225,325,425)) {
    par_to_fix$kphio           <- 0.0479684950570567
    par_to_fix$kphio_par_a     <- -0.00179211384220008
    par_to_fix$kphio_par_b     <- 18.4293950588911
    par_to_fix$soilm_thetastar <- 27.0859346061886
  ...
  if (scenario %in% c(1,2,11,12, 71,72, 81,82, 91,92, 111,112, 121,122, 221,222, 321,322, 421,422)){ # only GPP data
  ...
  } else if (scenario %in% c(426,427,428,           # the 220s are reruns from 2025-09-20))
                             326,327,328,           # the 220s are reruns from 2025-09-20))
    ...
    drivobs_train <- drivobs_train_bigD13C_vj_gpp |>
      unnest_wider(targets) |>
      filter(case_when(scenario %in% c(16,76,86,96,116, 126,226,326,426) ~ vj | bigD13C,
                       ...
                       scenario %in% c(            1270,127,227,327,427) ~ vj & !bigD13C,  # NOTE: this removes the three sites that have vj AND bigD13C
                       scenario %in% c(             129,128,228,328,428) ~ bigD13C & !vj,  # NOTE: this removes the three sites that have vj AND bigD13C
                       TRUE ~ TRUE)) |>
      nest(targets = c(vj, bigD13C, gpp))
    drivobs_test <- drivobs_test_bigD13C_vj_gpp ## for the test data set keep all
  } else if (scenario %in% c(0, 70, 80, 90, 110, 120, 220, 320, 420)) {  # only GPP data from FR-Pue
